...
variables:
  - group: azuredevops-test-for-integrations
...
class TestIntegrations:
    ...
    def test_through(self) -> None:
        ...
        variable_group = VariableGroup.create(self.ado_client, "azuredevops-test-for-integrations", "my_description", {"a": "b"})
        repo = Repo.create(self.ado_client, "azuredevops-test-for-integrations-repo")
        Commit.create(self.ado_client, repo.repo_id, "main", BRANCH_NAME, {"build.yaml": BUILD_YAML_FILE}, "add", "Initial commit")
        build_definition = BuildDefinition.create(self.ado_client, "azuredevops-test-for-integrations-build-definition", repo.repo_id, repo.name,
