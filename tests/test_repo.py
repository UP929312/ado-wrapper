...
from azuredevops.resources.pull_requests import PullRequest
...
class TestRepo:
    ...
    def test_create_delete(self) -> None:
        repo = Repo.create(self.ado_client, "azuredevops-test-repo-for-create-delete-repo")
        assert repo.name == "azuredevops-test-repo-for-create-delete-repo"
        repo.delete(self.ado_client)

    @pytest.mark.update
    def test_update(self) -> None:
        repo = Repo.create(self.ado_client, "azuredevops-test-repo-for-update-repo")
        Commit.create(
            self.ado_client, repo.repo_id, "main", "test-branch", {"test.txt": "Delete me!"}, "add", "Test commit"
        )  # Create a branch
        # =====
        repo.update(self.ado_client, "name", "azuredevops-test-repo-for-update-repo-renamed")
        assert repo.name == "azuredevops-test-repo-for-update-repo-renamed"  # Test instance attribute is updated
        ...
        assert fetched_repo.name == "azuredevops-test-repo-for-update-repo-renamed"
    ...
    def test_get_by_id(self) -> None:
        repo_created = Repo.create(self.ado_client, "azuredevops-test-repo-for-get-repo-by-id")
    ...
    def test_get_by_name(self) -> None:
        repo_created = Repo.create(self.ado_client, "azuredevops-test-repo-for-get-repo-by-name")
        repo = Repo.get_by_name(self.ado_client, "azuredevops-test-repo-for-get-repo-by-name")
        assert repo.name == repo_created.name
        assert repo.repo_id == repo_created.repo_id
        repo_created.delete(self.ado_client)

    def test_get_file(self) -> None:
        repo = Repo.create(self.ado_client, "azuredevops-test-repo-for-get-file")
    ...
    def test_get_contents(self) -> None:
        repo = Repo.create(self.ado_client, "azuredevops-test-repo-for-get-repo-contents")
    ...
    def test_get_pull_requests(self) -> None:
        repo = Repo.create(self.ado_client, "azuredevops-test-repo-for-get-pull-requests")
        Commit.create(
